...
// NewBookController is a function that creates a new instance of BookController
...
func (ctrl *BookController) SearchBooks(c *gin.Context) {
	...
	if err := ctrl.db.Where("title LIKE ? OR author LIKE ? OR publisher LIKE ? OR description LIKE ?", "%"+query+"%", "%"+query+"%", "%"+query+"%", "%"+query+"%").Find(&books).Error; err != nil {
		c.JSON(http.StatusInternalServerError, gin.H{"error": err.Error()})
		return
	}
...
func (ctrl *BookController) DeleteBook(c *gin.Context) {
	...
	var book models.Book
	if err := ctrl.db.Where("id = ?", bookID).First(&book).Error; err != nil {
...
func (ctrl *BookController) UpdateBook(c *gin.Context) {
	...
	// Check if the book with the given ID exists
	var book models.Book
	if err := ctrl.db.Where("id = ?", id).First(&book).Error; err != nil {
		c.JSON(http.StatusNotFound, gin.H{"error": "Book not found"})
		return
	}
